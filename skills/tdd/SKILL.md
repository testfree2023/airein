...
**台账义务**：仅 `tasks.md` 中 `Kind: implement` 的任务；Task 列 = Implement Task ID。标 `completed` 前须有合格行（Behavior + Test + Command 非空，Status=`pass`）。项目 `quality.json` → `testsLedger.enabled: true` 时由 hook 完成门禁强制；Verify/Deploy/Accept 不强制台账行。
...
5. **Trace** — 更新计划 `tests.md`（Task=本 Implement ID；Prove 后 Status=`pass`；删测试则删行或 `dropped`）
6. **Refactor** — 仅在绿灯下整理结构
7. **Coverage** — 按项目 `docs/test-plan.md` / quality 目标自检缺口
7.5. **Per-Task Code Review**（仅当 `quality.json` → `flowControl.perTaskReview` 为 `true` **且** `pipelineRoles.enabled` 不为 `false`）:
   - Dispatch `tech-lead`（**mode: review**；changed files only）
...
- **调用 `tech-lead`（**mode: review**）**（dispatch 规范见 `rules/20-workflow.md`：model 用 haiku；prompt 只写「mode: review — 审查当前 git diff」，**不粘贴 diff**）
- 若 `flowControl.perTaskReview === true` 且 `pipelineRoles.enabled !== false` → 每 task 完成后立即审查（dispatch tech-lead）
