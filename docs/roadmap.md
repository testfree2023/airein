...
- **[P003-multi-host-commands](plans/P003-multi-host-commands/)** — 多宿主 commands 部署（Multi-Host Commands Deployment）。状态：`in_progress`（2026-07-11 启动，m-feature pipeline；grilling completed，requirements draft；**dogfood 阻塞项 getProjectDir 已修**，下一步 requirements 审批 → design）。Priority: **P2**。触发：Cursor CLI 装 airein 后 `/tdd` `/plan` `/verify` 等 16 个 slash command 不可用——P001 `install-host.js` 三支柱（K1 skills/K2 rules/K3 hooks）遗漏第四类资产 commands。范围：**K4 command-place**（`scripts/lib/command-place.js` 仿 K1 `skill-place.js`）→ 仓库 `commands/*.md` 原样拷贝到三宿主发现路径（Cursor `.cursor/commands/` + CodeBuddy `.codebuddy/commands/` + OpenCode `commands/`）；Codex N/A（`~/.codex/prompts/` OpenAI 标 deprecated + bug #15941）；CC 不动（`~/.claude/commands/` 由 setup-airein.sh 管）。install-host.js 加 K4 编排 + verify-airein.sh --host 加 commands 产物校验 + install-hosts.md 补 commands 矩阵。commands 内容零改动（单一真相源，跨宿主兼容性靠真机冒烟 deferred，同 P001 T12）。复用 P001 install-host.js 框架 + install-manifest hash + deployment §3 矩阵。
...
- [ ] **安装部署验证缺失**：当前无 CI、无自动化端到端安装测试；行为层验证靠人工按 `test-plan.md` 执行。需补 CI 流水线 + 真实安装冒烟测试。
  > 进展（2026-07-09）：首次真实安装已人工验证通过（macOS，见 Recent Changes）；`verify-airein.sh` 自身回归测试已补（2026-07-10，`test/test-verify-airein.js` 14 用例 node 驱动 bash，覆盖 4 宿主 install→verify + 删产物报缺失 + 前置错误 + CC 模式不回归）；CI / 自动化 4 宿主真机冒烟仍缺。
...
- [ ] **plan-gate 盲区（dogfood · P001 归档发现 · 2026-07-11）**：`isPlanCompleted` 把 task 全完成但 `status=in_progress` 的「待归档」计划判为 `completed` → 归档前想补修复却被 plan-gate 当「completed plan 不该再改源码」锁住。归档应是显式状态翻转，不该被任务完成度推断覆盖。修方向：plan-gate / isPlanCompleted 区分「task 完成」与「plan archived」，`status=in_progress` 时允许归档前修复。
- [ ] **doc-file-warning 豁免缺 `SECURITY.md`（dogfood · 2026-07-11 安全加固发现）**：根目录白名单正则（`scripts/hooks/doc-file-warning.js:29`）含 `README|CLAUDE|AGENTS|CONTRIBUTING|CHANGELOG|RELEASES|LICENSE|SKILL|MEMORY|WORKLOG|CONTEXT` 但缺 `SECURITY` → 本次安全加固写根 `SECURITY.md` 被 exit 2 硬拦（临时放 `docs/SECURITY.md` 绕过）。`SECURITY.md` 是 GitHub/OSI 约定的**根目录**文件（GitHub Insights>Security 自动识别根 `SECURITY.md`），长期应居根而非 docs/。修：白名单加 `SECURITY`（与 README/LICENSE 同属「根级约定文档」）+ `test-doc-file-warning.js` 补断言。同类候选：根级 `CODE_OF_CONDUCT.md` / `SUPPORT.md`（GitHub 同样自动识别根目录）。
