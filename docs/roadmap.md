...
- **[P008-pipeline-roles](plans/P008-pipeline-roles/)** — **Agent Teams v0**。状态：completed（17/17，待归档/PR）| Priority: **P2** | m-feature。分支：feat/p008-pipeline-roles。

- **[P007-task-pickup-progress-sync](plans/P007-task-pickup-progress-sync/)** — 任务面板优化 + progress-sync / 派工（tasks 为源）。状态：`completed`（9/9，待归档）| Priority: **P2** | s-feature。产物：`task-pickup` + `progress-from-tasks`；progress-sync / session-start 双触发空档标 `in_progress`；`taskPickup.onBlocked`；面板 `is-current`。**不含** watchdog；Agent Teams 另文本地跟踪（不上游）。详见 plan 目录。
- **[P006-dashboard-progress-panel](plans/P006-dashboard-progress-panel/)** — Dashboard Progress 任务进展面板 + tasks.md 可解析契约。状态：`completed`（6/6，待归档）| Priority: **P2** | s-feature。产物：`parse-tasks-panel` + Progress 双 Tab（面板/文本）+ 模板面板契约；旧模板提示「老的任务模板暂不支持」。详见 plan 目录。
...
- **[P009-marketplace-readiness](plans/P009-marketplace-readiness/)** — 市场上架准备：体验门槛 + MVP（R1 三车道）。状态：`archived`（2026-07-21，19/19；Anthropic Plugin Directory pending review；macOS E3 未跑 → 不标硬上架）。Priority: **P2** | m-feature。产物：`.claude-plugin/` L1 thin bridge、`kernel-ready`、三车道 README、SUPPORT/SLA；提交证据见 plan progress Notes。
...
- **2026-07-21** `release` VERSION **2.06 → 2.07** —— progress 审批门禁（Status 先行 + tasks 格式只卡审批）+ 货架/一键安装 UX + P009 L1 plugin 归档合入。
- **2026-07-21** `archive` 归档 P009 市场上架准备（19/19）—— Anthropic 提交 pending review；三车道 + L1 plugin bridge + kernel-ready；**不**标硬上架（macOS E3 未跑）。
- **2026-07-20** plan 启动 P009 市场上架准备（体验门槛 + 上架 MVP）—— grilling 进行中。前置：货架基建 topics/homepage/v2.06 Release 已用 Playwright 落地；本地货架文档待合入。
- **2026-07-20** `release` VERSION **2.05 → 2.06** —— 项目首页文档卡改版 + README 文档模板说明；CHANGELOG 收纳 P006/P007/P008 与角色收敛等 Unreleased 条目。
- **2026-07-18** plan 启动 P008 Pipeline 角色编排（PM=main + Tech Lead）—— 分支 eat/p008-pipeline-roles；grilling 进行中。
- **2026-07-18** `refactor` 角色收敛：`architect` / `code-reviewer` / `security-reviewer` → 单一 `tech-lead`（design/review/security）；铁律与 `/code-review` / tdd 终止态同步。
- **2026-07-18** `chore` ECC 白名单内化：重写 `architect` / `code-reviewer` / `security-reviewer` 与 `/tdd` `/code-review` `/verify`；删除 `tdd-guide`（卡住走 `skills/tdd`）；README 致谢改「曾使用」。
- **2026-07-18** `chore` ECC 表面第一刀收缩：`agents/` 仅留 `architect` / `code-reviewer` / `security-reviewer` / `tdd-guide`；`commands/` 仅留 `tdd` / `code-review` / `verify`；同步 `manage-profile` / `install-hosts` / README 致谢与契约测试。产品定位：宿主内 skills/plugin（文档+规格驱动进展），非通用 Agent 全家桶。
- **2026-07-18** `docs` 引入根目录 [CHANGELOG.md](../CHANGELOG.md)——用户向发布摘要（plan completed / archive-plan 必写）；roadmap Recent Changes 仍为开发过程日志。二者分工，勿混写。
- **2026-07-18** `feat` 完成 P007 任务面板优化 + 派工同步（s-feature，9/9）—— `task-pickup` / `progress-from-tasks`；progress-sync 与 session-start 双触发；`taskPickup.onBlocked`；面板当前任务高亮。无 watchdog。下一步：归档 / 本地 sync dogfood。
- **2026-07-18** `docs` Agent Teams 需求分析（skills 自建团队；不做会话外 watchdog；与 P007 切分）—— **仅本地**，不上游。
- **2026-07-18** `plan` 收窄 P007 为任务面板优化 + 派工同步（去 watchdog；requirements 重拟 draft）—— Agent Teams 另文跟踪。
- **2026-07-18** `plan` 启动 P007 任务派工 + progress-sync（s-feature）—— 触发：P006 后仍缺可靠「下一任务」与 Active Task 同步；用户确认 **提示 + 自动标 in_progress** 一起做。
- **2026-07-17** `feat` 完成 P006 Dashboard Progress 任务进展面板 + tasks 模板契约（s-feature，6/6）—— `scripts/lib/parse-tasks-panel.js` 面板契约解析；Progress 双 Tab（只读面板/文本）；旧模板「老的任务模板暂不支持」；`templates/docs/tasks.md` Status/Depends on/Accept 契约。下一步：归档 / PR。
- **2026-07-17** `plan` 启动 P006 Dashboard Progress 任务进展面板 + tasks 模板契约（s-feature）—— 触发：执行态无法直观看任务顺序/依赖/节点进展；模板机读纪律不足。grilling 已完成（只读双 Tab、状态着色、文档序、模板与面板同计划）。下一步：requirements 审批 → tasks。
