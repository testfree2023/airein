...
**Airein** 是套在 Claude Code（及其它宿主）之内的 **skills / plugin 工程化层**（非宿主替代品，也非全家桶工具箱）；其核心是把 AI 编码助手，把 AI 编码助手从"按提示生成代码的玩具"驯化成"懂规范、能闭环、会自学的入职半年同事"。

核心理念一句话：**Prompt 是建议，Hook 是法律。**

## 不是什么

- ❌ 不是 Claude Code / Cursor 等宿主的替代品（在它们之内工作，不替代）
- ❌ **不是**用户 AI 编程工具的全家桶管家；**只是**宿主里的 **skills / plugin 一层**
- ❌ 不是又一套「大而全」 Agent / slash command 合集（通用角色卡、通用工具命令交给用户自选或自装 ECC 等）
- ❌ 不是又一套 Cursor Rules（不是 prompt 集合，是工程系统：文档流水线 + hook 门禁）
...
- AI 模型本身的调用逻辑（不改变宿主的模型行为）
- 替用户管理整个 AI 编程工具（通用 Agent 角色卡合集、通用 slash command 工具箱等）
- 真人团队协同 SaaS（RBAC、审计日志、合规模板）——可能另立项目
- 企业版功能
