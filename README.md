...
| 项目 | 关系 | 说明 | 链接 |
|------|------|------|------|
| **Everything Claude Code (ECC)** | **曾使用** + 参考 | **曾使用**：早期内置 `tdd-workflow` / `verification-loop` 改编自 ECC；现已分别内核化为自有 skill `tdd`（规格绑定 + 计划 `tests.md` 台账），以及内化到 `rules/20-workflow.md` 的完成前门禁。**参考**：整体架构与 hook 事件划分。 | [github.com/affaan-m/everything-claude-code](https://github.com/affaan-m/everything-claude-code) |
...
**边界**：hooks / scripts / 全部现编 skill（含 `tdd`、`new-plan` 等）为 airein 自研或已内核化文本；运行时**零 npm 依赖**，只用 Node.js 内建模块。致谢「曾使用」表示历史出处，不等于当前仍依赖对方仓库或 npm 包。
