...
```bash
bash airein-pack.sh /path/to/output              # 打包
scp airein-*.tar.gz user@newhost:~/         # 拷贝
bash airein-unpack.sh airein-*.tar.gz      # 解包
```
...
**Q: 换电脑后自学习记忆会丢吗？**
A: 自学习 archive 在 `~/.claude/projects/{key}/self-learning-archive.md`，按项目隔离、不走 git。迁移时手动复制该目录，或用 `airein-pack.sh` 打包。
...
Airein 遵循「零外部依赖」原则，上述借鉴均为**设计理念与方法论参考**，未引入任何运行时依赖库。
