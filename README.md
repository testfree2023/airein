...
**当前版本：[2.06](CHANGELOG.md)** · [30 秒安装](#30-秒安装) · [演示走查](docs/demo.md) · [安全](docs/SECURITY.md) · [支持与已知限制](SUPPORT.md)

**Airein** = **AI** + **rein**（缰绳）。名字即目标：给 AI 编程套上可握的缰绳——用规格与 hook 驾驭复杂交付，而不是放任模型信马由缰。

---

## 30 秒安装

一句话定位：**规格驱动的文档与进展 + 代码强制的 hook 门禁**（Prompt 是建议，Hook 是法律）。

一条命令完成**完整安装**（拉取仓库 → `airein setup --yes` → 写入 `~/.airein` 并注册已检测到的宿主；**不是**只装 plugin 文案）：
...
可选：Unix 指定宿主 `curl … | bash -s -- --hosts claude-code`；Windows 可先设 `$env:AIREIN_HOSTS='claude-code'` 再 `irm … | iex`。

验证：`bash ~/.airein/scripts/update/verify-airein.sh --full`（推荐）。仅 Claude Code / 仅 Cursor 见下文「5 分钟上手」。

> **供应链提示**：上述命令会从 GitHub 下载并执行 bootstrap（业界常见的 `curl | bash` / `irm | iex`）。组织若要求审阅，请用下方「可审计安装」，或先打开 [`scripts/install.sh`](scripts/install.sh) / [`scripts/install.ps1`](scripts/install.ps1)。

<details>
<summary>可审计安装（先 clone 再 setup）</summary>
...
> **不要把 `npx skills add …` 当成完整安装。** 它通常只拷贝 skill 文案，**不会**部署 hooks / `~/.airein` 内核。完整产品请用上面的一键安装或 `airein setup`。

### 安装三车道（勿混）

| 车道 | 通道 | 你得到什么 |
|------|------|------------|
| **完整（推荐）** | `airein setup` | `~/.airein` 内核 + 宿主 hooks（唯一完整产品） |
| **CC 商店发现** | Claude Code `/plugin install`（本仓 `.claude-plugin/`） | skills/commands + 缺核提示；**装完仍须 `airein setup`** |
| **仅发现** | `npx skills add` / skills.sh | **不完整** skill 文案；不得当完整安装 |

Cursor **不是** Claude Code Plugin 通道：请用 `airein setup --hosts cursor`。勿以为「一份 CC plugin = 全宿主开箱」。

**信任要点**：安装会写入宿主 hook 配置，并在本地执行内核 `scripts/`（**零 npm 依赖**，但仍建议按需审阅）。默认本地运行、不上传遥测。详见 [SECURITY](docs/SECURITY.md) 与 [SUPPORT](SUPPORT.md)。演示步骤：[docs/demo.md](docs/demo.md)（录屏/GIF 可按该脚本补 `docs/assets/demo.gif`）。

**卸载**：`airein uninstall`（详见 [SUPPORT](SUPPORT.md) · [deployment](docs/deployment.md)）。上架前冷启动记录模板：[sla-ledger](docs/plans/P009-marketplace-readiness/sla-ledger.md)。
...
Airein **不是**替你管理整个 AI 编程工具的全家桶；它只是宿主（Claude Code / Cursor 等）里的 **skills / plugin 一层**。我们要做好的是：**项目文档管理 + 规格驱动进展**（init-project / new-plan / 审批与 hook 门禁 / Dashboard 等）——不是堆一大堆「看起来都有用」的通用 Agent / command。那些交给你自选工具，或自行安装 [ECC](https://github.com/affaan-m/everything-claude-code) 等插件。 写在 CLAUDE.md 里的规则是"建议"——模型读到、多数遵守，但会被上下文淹没、被绕过、被遗忘；而 hook 的 `exit 2` 是"法律"——代码保证，违反即阻断，不依赖模型自觉。能 hook 强制的，就不只靠 prompt。
...
**Q: 已知限制有哪些？多宿主都开箱即用吗？**
A: 首版完整支持 **Claude Code + Cursor**；Codex / CodeBuddy / OpenCode 为预览。Windows 请用 Git Bash。安全上报与信任模型见 [SUPPORT.md](SUPPORT.md) / [docs/SECURITY.md](docs/SECURITY.md)。演示走查见 [docs/demo.md](docs/demo.md)。
