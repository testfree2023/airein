...
function getChecks() {
  ...
  const commandPrimary = safeRead('commands/airein-audit.md').trim();
  const commandParity = safeRead('.opencode/commands/airein-audit.md').trim();
  ...
  return [
    ...
    },
    {
      ...
      path: '.opencode/commands/airein-audit.md',
      description: 'Airein-audit command parity exists between primary and OpenCode command docs',
      pass: commandPrimary.length > 0 && commandPrimary === commandParity,
      fix: 'Sync commands/airein-audit.md and .opencode/commands/airein-audit.md.',
    },
    {
    ...
    },
    {
      ...
      points: 4,
      scopes: ['repo', 'skills'],
      path: 'skills/eval-airein/SKILL.md',
      description: 'Eval airein skill exists',
      pass: fileExists('skills/eval-airein/SKILL.md'),
      fix: 'Add skills/eval-airein/SKILL.md for pass/fail regression evaluation.',
    },
    {
      id: 'eval-commands',
      category: 'Eval Coverage',
...
function printText(report) {
  console.log(`Airein Audit (${report.scope}): ${report.overall_score}/${report.max_score}`);
  console.log('');
...
function showHelp(exitCode = 0) {
  console.log(`
Usage: node scripts/airein-audit.js [scope] [--scope <repo|hooks|skills|commands|agents>] [--format <text|json>]

Deterministic airein audit based on explicit file/rule checks.
`);
